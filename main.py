...
def get_data(data_dir, source, target, height, width, batch_size, re=0, workers=8):
    ...
    source_train_loader = DataLoader(train_set,
                                     batch_size=batch_size, num_workers=workers, sampler=RandomSampler(train_set, 6),
    ...
    target_train_loader = DataLoader(
        UnsupervisedCamStylePreprocessor(dataset.target_train,
                                         ...
                                         camstyle_root=osp.join(dataset.target_images_dir, dataset.target_train_camstyle_path),
...
def main(args):
    ...
    target_classifier = nonparametric_classifier(args.features, num_tgt, tau=args.tau, lambda0=args.lambda0, delta=args.delta,
                                                 mu=args.mu)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--target', type=str, default='market',
                        choices=['market', 'duke', 'msmt17'])
    # imgs setting
    parser.add_argument('-b', '--batch-size', type=int, default=192)
